...
> 版本：v0.1（设计 + 实现说明：阶段 1-3 已完成，阶段 4 已部分实施）  
...
- 中间帧文件清理：
  - 导出任务无论成功、失败或被取消，都会在最终阶段调用 `clearExportFrames` 删除 `/export/frame-*.png` 中间文件，避免长时间占用 ffmpeg 虚拟 FS。
...
- 阶段 4：性能优化与高级特性
  - 已部分实施：
    - 导出前基于 `frameCount` 做时长阈值（约 60 秒）判断，并通过确认弹窗提示用户导出可能较慢；
    - 在离线帧渲染循环中加入时间切片逻辑，避免长时间阻塞主线程；
    - 导出结束（无论成功或被取消）后清理 `/export/` 目录下的临时帧文件（`frame-*.png`），减少 FFmpeg FS 占用。
...
- UI 形态：
 ...
 - 错误提示方式：
  - 设计稿仅要求在导出失败时给予提示；
  - 当前实现中，当导出流程出现非用户取消的异常时，会通过 Element Plus 的 `ElMessage.error` 在页面右上角弹出统一的“导出失败，请稍后重试”提示；用户主动取消导出则不弹出错误消息。
