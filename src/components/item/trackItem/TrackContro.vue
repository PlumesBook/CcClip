  ...
  <div class="pl-4 pb-1 pr-10 w-full h-10 border-b dark:border-gray-600 border-gray-300">
    <div class="float-left h-9 w-32 flex flex-row flex-nowrap items-center justify-around">
      <div v-for="item of icons" :key="item.title" @click="handlerIcon(item)">
        <el-tooltip :disabled="item.disable" class="bg-gray-400" :effect="store.isDark ? 'dark' : 'light'"
          :content="item.title" placement="bottom-start">
          <component :is="item.icon" class="focus:outline-0"
...
function handlerIcon(item: Record<string, any>) {
  ...
  if (type === 'DeleteIcon') {
    ...
    // Check if split time is within the active item's range
    if (splitTime > active.start && splitTime < active.end) {
      let copy = JSON.parse(JSON.stringify(active));

      const originalEnd = active.end;
      const originalStart = active.start;

      // Update Left Item (Active)
      active.end = splitTime;
      active.offsetR += (originalEnd - splitTime);

      // Update Right Item (Copy)
      copy.start = splitTime;
      copy.offsetL += (splitTime - originalStart);

      copy.id = getId();

      // Copy attributes
      const originalAttrs = attrStore.trackAttrMap[active.id];
      if (originalAttrs) {
        const copiedAttrs = JSON.parse(JSON.stringify(originalAttrs));
        attrStore.setTrackAttr(copy.id, copiedAttrs);
      }
