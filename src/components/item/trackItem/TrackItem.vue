<template>
  <div class="text-left text-sm top-0 absolute "
    :class="[TrackHeightMap.get(props.trackItem.type), isDragState ? 'opacity-30' : '']" :style="itemClass"
    @click="setSelectTract" @contextmenu.prevent.stop="handleContextMenu">
    <!-- 操作手柄 -->
    ...
    <div v-if="showMenu"
      ...
      <template v-if="canReplace">
        <div class="px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer flex justify-between items-center"
    ...
    <!-- 资源选择弹窗 -->
    <ResourceSelectDialog v-if="canReplace" v-model="showResourceDialog" :resource-type="trackItem.type"
      @select="handleResourceSelect" />

    <!-- 隐藏的文件输入框 -->
    <input ref="fileInput" type="file" class="hidden" :accept="acceptType" @click.stop @change="handleFileChange" />
  </div>
</template>
